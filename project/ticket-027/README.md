...
The adoption is limited to governance and host continuity files. Application
source, tests, dependencies, secrets and provider configuration remain outside
the accepted scope.

## Acceptance criteria

- [ ] AC-01: The read-only adoption preflight identifies the published immutable
  standard revision and no managed target is blocked by Git ignore rules.
- [ ] AC-02: Goal upgrades the managed governance package to new-project 0.20.31
  and tracks `.subactor/manifest.json` and `.github/copilot-instructions.md`.
